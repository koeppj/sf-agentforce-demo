...
6. **Blocked:** Build and test the Add and Remove flows, including duplicate and confirmation paths. The required `execute_metadata_action` Flow-generation service is not available in the current workspace; see `FLOW_GENERATION_SERVICE_REQUIREMENT.md`.
7. **Pending:** Add flow-access entries to `Case_Team_Assignment_Manager`, deploy both flows and the updated permission set, then run the acceptance tests above.
